+++
title = "Fast Inference Might Matter More Than Smarter Models Now"
date = 2026-07-02
draft = false
tags = ["ai"]
+++

I have a feeling that the next breakthroughs in AI won’t necessarily come from more capable models, but rather from much faster inference. In the past two weeks, we’ve seen three interesting developments in that direction.

First, [OpenAI announced GPT-5.6 Sol](https://openai.com/index/previewing-gpt-5-6-sol/):

> "We're also launching GPT‑5.6 Sol on Cerebras at up to 750 tokens per second in July"

Second, [Google released Nano Banana 2 Lite](https://deepmind.google/models/gemini-image/flash-lite/), bringing image generation down to just a few seconds. Finally, they also published [Diffusion Gemma](https://deepmind.google/models/gemma/diffusiongemma/), achieving over 1,000 tokens per second on a single NVIDIA H100 GPU.

I suspect that even with model capabilities around the level of [Claude Opus 4.6+](https://www.anthropic.com/news/claude-opus-4-6), this kind of inference speed would enable a lot of new use cases, much like video streaming only became practical once the internet became fast enough.
